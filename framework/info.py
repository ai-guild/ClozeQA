...
def highlight_answer(sample):
    # iterate context word by word
    candidates = sample['candidates']
    answer = sample['answer']
    for w in word_tokenize(sample['context']):
        # if candidate -> use Green
        if w in candidates and w != answer:
            Gp(w, e=' ')
        if w == answer: # if answer -> use Magenta
            Mp(w, e=' ')
        else:
            print(w, end=' ')
    print('\n')
